 ...
 *
 * Reads the local performance DB + guardrail settings and prints proposed
...
const todaySpend = db
  .prepare("SELECT COALESCE(SUM(spend_cents),0) c FROM metrics_daily WHERE date = ?")
...
const ads = db
  .prepare(
    `SELECT a.id, a.name, a.status, a.engine_managed, a.creative_id,
     ...
     WHERE a.status IN ('active','paused') AND a.fb_ad_id IS NOT NULL`
  )
  .all() as any[];
...
for (const ad of ads) {
  ...
  const last3 = db
    .prepare(
      `SELECT date, CASE WHEN impressions>0 THEN 100.0*clicks/impressions ELSE 0 END ctr
       FROM metrics_daily WHERE ad_id = ? ORDER BY date DESC LIMIT 3`
    )
    .all(ad.id) as { date: string; ctr: number }[];
  const decaying =
  ...
  if (beatingCpa || strongCtr) {
    const recentScale = db
      .prepare(
        `SELECT COUNT(*) c FROM engine_actions
         WHERE ad_id = ? AND action='scale_budget' AND executed=1
         AND created_at > datetime('now','-1 day')`
      )
      .get(ad.id) as { c: number };
...
for (const a of actions) {
  const info = insert.run(
    a.ad_id, a.campaign_id, a.action, a.reason, JSON.stringify({ ...a.metrics_snapshot, params: a.params ?? null })
  );
  proposalIds.push(Number(info.lastInsertRowid));
}
